  ...
  module TermValueOperators
    ...
    def build_ancestors(parent_select, parent_index)
      ...
      to_build.each do |term_pointer|
        ...
        if find_by_terms(*parent_select+[{}]).length == 0 && !(parent_select.first.kind_of?(Hash) && (parent_select.first.has_key?(:mods) || parent_select.first.has_key?(:pageData)))
