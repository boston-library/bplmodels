    ...
    def to_solr(doc = {} )
      ...
      if self.descMetadata.subject.cartographics.coordinates.length > 0
        self.descMetadata.subject.cartographics.coordinates.each do |coordinates|
          if coordinates.scan(/[\s]/).length == 3
            # don't want 'whole world' boxes
            unless coordinates == '-180.0 -90.0 180.0 90.0'
              doc['subject_bbox_geospatial'] ||= []
              doc['subject_coordinates_geospatial'] ||= []
              bbox_to_env = Bplmodels::GeographicDataFuncs.bbox_formatter(coordinates, 'wkt_envelope')
              doc['subject_bbox_geospatial'] << bbox_to_env
              doc['subject_coordinates_geospatial'] << bbox_to_env
            end
          else
      ...
      self.descMetadata.mods(0).title.each_with_index do |title_value,index|
        ...
        if self.descMetadata.mods(0).title_info(index).usage[0] == 'primary'
          ...
          doc['title_info_uniform_ssim'] << title_prefix + title_value
...
=end
    ...
    def export_for_bpl_api
      export = {}
      # we could potentially use results of to_solr for a lot of this stuff
      @solr_hash = to_solr
      ...
      export[:metastreams] = {}
      export[:metastreams][:descriptive] = {
    ...
    def titles_for_export_hash
      ...
      descMetadata.mods(0).title.each_with_index do |title, index|
        ...
        title_hash.select! { |_k, v| v } # remove nil values
    ...
    def names_for_export_hash
      ...
      descMetadata.mods(0).name.each_with_index do |_name, index|
        ...
        descMetadata.mods(0).name(index).role.each_with_index do |_role, role_index|
          ...
          roles << role_hash.select { |_k, v| v }
        end
        roles.each do |role_hash|
          names << { name: name_hash.select { |_k, v| v }, role: role_hash}
        end
      end
      names
    end
    ...
    def genres_for_export_hash
      ...
      descMetadata.mods(0).genre.each_with_index do |genre, index|
        ...
        genres << genre_hash.select { |_k, v| v }
      end
      genres
    end
